      ...
      <div className={`rounded-2xl border-2 transition-all ${
        ...
        <div className="p-5 flex items-start gap-4">
          ...
          <button
            ...
            className={`flex flex-col items-center gap-0.5 shrink-0 pt-0.5 transition-colors rounded-lg p-1 ${
              hasVoted
                ? 'text-honeycomb'
                : 'text-denim/40 hover:text-denim hover:bg-powder'
            }`}
              ...
              <div className="space-y-3">
                {answers.map(a => {
                  ...
                  return (
                    <div
                      key={a.id}
                      className={`rounded-xl p-4 ${coachAnswer ? 'bg-atlantic-navy' : 'bg-powder/50'}`}
                    >
            ...
            {!q.is_resolved && (
              <div className="space-y-2">
                ...
                <textarea
                  rows={3}
                  value={af.body}
                  onChange={e => setAnswerField(q.id, 'body', e.target.value)}
                  className="input-field resize-none text-sm"
                  placeholder={
                    isCoach
                      ? 'Type your answer here — or just drop a link below.'
                ...
                <input
                  type="url"
                  value={af.link}
                  onChange={e => setAnswerField(q.id, 'link', e.target.value)}
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-denim mt-1">
            {isCoach
              ? 'Answer student questions with text or a video link. Questions with 🔥 are the most common.'
        ...
        <div className="space-y-10">
          {open.length > 0 && (
            <section>
              ...
              <div className="space-y-3">
                {open.map(q => <QuestionBlock key={q.id} q={q} />)}
              </div>
            </section>
          )}
          ...
          {resolved.length > 0 && (
            <section>
              ...
              <div className="space-y-3">
                {resolved.map(q => <QuestionBlock key={q.id} q={q} />)}
              </div>
            </section>
          )}
        </div>
